...
class RangeNetInference(Node):
    def __init__(self):
        super().__init__('rangenet_inference')
        ...
        self.label_pub = self.create_publisher(
            PointCloud2, topic_cfg.get('output_labels', '/rangenet/labeled_cloud'), 10)
    ...
    def load_model(self):
        model_path = "/home/ronak/3D/ouster_os0/ouster_os0/src/models/trained/rangenetpp_sequence_00/rangenetpp_best.pth"    
        self.get_logger().info(f'Loading model from {model_path}')
        checkpoint = torch.load(model_path, map_location='cpu', weights_only=False)
        
        arch = checkpoint.get('architecture', 'rangenetpp')
        if arch != 'rangenetpp':
            self.get_logger().warn(f"Checkpoint architecture is '{arch}', but forcing to 'rangenetpp'")
            arch = 'rangenetpp'
